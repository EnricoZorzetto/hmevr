...
#' Compute yearly rainfall statistics from a time series
#'
#' @param df A data frame with the data.
#' @param Nt = 366 The number of events / year.
#' @param reshuffle_days = FALSE wether or not reshuffle the days.
#' @return a list with various statistics.
#' @export
table_max <- function(df, Nt = 366,
  ...
  ----------------------------------------------------
  Nt (default Nt = 365) Number of events / block
  in the output matrix

  reshuffle_days -> if true, the number of events/block
  ...
  the vector N with the number of events > 0 / year
...
# dhbev <- function(model_fit, Xi){
#   "---------------------------------------------------------------------
#   compute the hbev density for the scalar value of arrays of values Xi
#   return their corresponding pdf
#   model_fit -> output list from the fit_ev_model function
#   ----------------------------------------------------------------------"
#   pdf = 0
#   return(pdf)
# }

# qhbev <- function(model_fit, Xi){
#   "---------------------------------------------------------------------
#   compute the hbev quantiles for the scalar value of arrays of values
#   either Fi (non exceedances) or
#          Gi (survival probabilities) or
#          Tr (return times)
#
#   return their corresponding pdf
#   model_fit -> output list from the fit_ev_model function
...
# }
  ...
  defp = list(
    ...
    # weibull models
      inf_mc0 = 30, # the larger the more informative the prior
      inf_sc0 = 300,
      inf_mw0 = 30,
      inf_sw0 = 300,
...
wsnsh0prior  = c(defp[['inf_snsh']]*defp[['sn_shape_w']], defp[['inf_snsh']])    # gamma a, b
  ...
  if (x_mod == 'wei_dyn' || x_mod == 'gga_dyn' || x_mod == 'gga_sta' || x_mod == 'cop_dyn' || x_mod == 'wei_dyl'){
